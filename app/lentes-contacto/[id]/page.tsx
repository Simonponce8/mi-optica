...
export default function EditarLenteContacto() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      const { data: lente } = await supabase.from("lentes_contacto").select("*").eq("id", params.id).single();
      if (lente) {
        setClienteId(lente.cliente_id);
        const { data: cl } = await supabase.from("clientes").select("*").eq("id", lente.cliente_id).single();
        if (cl) setCliente(cl);
        setForm({
          fecha: lente.fecha || "",
          doctor: lente.doctor || "",
          aereo_od_esfera: lente.aereo_od_esfera !== null && lente.aereo_od_esfera !== undefined ? formatGrad(lente.aereo_od_esfera) : "",
          aereo_od_cilindro: lente.aereo_od_cilindro !== null && lente.aereo_od_cilindro !== undefined ? formatGrad(lente.aereo_od_cilindro) : "",
          aereo_od_eje: lente.aereo_od_eje ?? "",
          aereo_od_av: lente.aereo_od_av || "",
          aereo_oi_esfera: lente.aereo_oi_esfera !== null && lente.aereo_oi_esfera !== undefined ? formatGrad(lente.aereo_oi_esfera) : "",
          aereo_oi_cilindro: lente.aereo_oi_cilindro !== null && lente.aereo_oi_cilindro !== undefined ? formatGrad(lente.aereo_oi_cilindro) : "",
          aereo_oi_eje: lente.aereo_oi_eje ?? "",
          aereo_oi_av: lente.aereo_oi_av || "",
          aereo_ao_av: lente.aereo_ao_av || "",
          aereo_add: lente.aereo_add || "",
          final_od_esfera: lente.final_od_esfera !== null && lente.final_od_esfera !== undefined ? formatGrad(lente.final_od_esfera) : "",
          final_od_cilindro: lente.final_od_cilindro !== null && lente.final_od_cilindro !== undefined ? formatGrad(lente.final_od_cilindro) : "",
          final_od_eje: lente.final_od_eje ?? "",
          final_od_av: lente.final_od_av || "",
          final_oi_esfera: lente.final_oi_esfera !== null && lente.final_oi_esfera !== undefined ? formatGrad(lente.final_oi_esfera) : "",
          final_oi_cilindro: lente.final_oi_cilindro !== null && lente.final_oi_cilindro !== undefined ? formatGrad(lente.final_oi_cilindro) : "",
          final_oi_eje: lente.final_oi_eje ?? "",
          final_oi_av: lente.final_oi_av || "",
          final_ao_av: lente.final_ao_av || "",
          final_add: lente.final_add || "",
          prueba_1: lente.prueba_1 || "",
          observaciones: lente.observaciones || "",
        });
      }
  ...
  const handleGuardar = async () => {
    setLoading(true);
    const datos = { fecha: form.fecha, doctor: form.doctor };
    ...
    const { error } = await supabase.from("lentes_contacto").update(datos).eq("id", params.id);
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f4f7fb" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        <div style={{ background: "#fff", borderBottom: "1px solid #dde3ec", padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: "15px", fontWeight: "600", color: "#1e3a5f" }}>Lente de contacto</span>
          <button onClick={() => router.push("/clientes/" + clienteId)} style={{ background: "transparent", color: "#6b7a8f", border: "1px solid #dde3ec", borderRadius: "6px", padding: "8px 16px", cursor: "pointer", fontSize: "13px" }}>
            Cancelar
          </button>
        </div>
        ...
        <div style={{ padding: "24px", maxWidth: "700px" }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #dde3ec", borderRadius: "8px", padding: "20px", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {cliente && (
              <div style={{ background: "#E6F1FB", borderRadius: "8px", padding: "14px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", fontSize: "13px" }}>
                <div>
                  <div style={{ color: "#185FA5", fontSize: "11px", marginBottom: "2px" }}>Paciente</div>
                  <div style={{ color: "#0C447C", fontWeight: "600" }}>{cliente.apellido}, {cliente.nombre}</div>
                </div>
                <div>
                  <div style={{ color: "#185FA5", fontSize: "11px", marginBottom: "2px" }}>Edad</div>
                  <div style={{ color: "#0C447C", fontWeight: "600" }}>{calcularEdad(cliente.fecha_nacimiento) !== null ? calcularEdad(cliente.fecha_nacimiento) + " anos" : "-"}</div>
                </div>
                <div>
                  <div style={{ color: "#185FA5", fontSize: "11px", marginBottom: "2px" }}>DNI</div>
                  <div style={{ color: "#0C447C", fontWeight: "600" }}>{cliente.dni || "-"}</div>
                </div>
              </div>
            )}
