...
export default function NuevaEvaluacion() {
  ...
  const [nuevoCliente, setNuevoCliente] = useState({ nombre: "", apellido: "", dni: "", telefono: "", email: "", fecha_nacimiento: "", obra_social: "", observaciones: "" });
  ...
  const crearCliente = async () => {
    ...
    const datos = { nombre: nuevoCliente.nombre, apellido: nuevoCliente.apellido };
    if (nuevoCliente.dni) datos.dni = nuevoCliente.dni;
    if (nuevoCliente.telefono) datos.telefono = nuevoCliente.telefono;
    if (nuevoCliente.email) datos.email = nuevoCliente.email;
    if (nuevoCliente.fecha_nacimiento) datos.fecha_nacimiento = nuevoCliente.fecha_nacimiento;
    if (nuevoCliente.obra_social) datos.obra_social = nuevoCliente.obra_social;
    if (nuevoCliente.observaciones) datos.observaciones = nuevoCliente.observaciones;
    ...
    const { data: clientesActualizados } = await supabase.from("clientes").select("id, nombre, apellido").order("apellido");
    if (clientesActualizados) setClientes(clientesActualizados);
    setClienteId(data.id);
    setNuevoCliente({ nombre: "", apellido: "", dni: "", telefono: "", email: "", fecha_nacimiento: "", obra_social: "", observaciones: "" });
  ...
  const handleGuardar = async () => {
    ...
    const { error } = await supabase.from("evaluaciones_neurosensoriales").insert([datos]);
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f4f7fb" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        <div style={{ background: "#fff", borderBottom: "1px solid #dde3ec", padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: "15px", fontWeight: "600", color: "#1e3a5f" }}>Nueva Evaluacion Neurosensorial</span>
          ...
          <Seccion titulo="Datos Generales" abierto={abiertos.datos} onToggle={() => toggle("datos")}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", marginBottom: "14px" }}>
              <div>
                {lbl("Paciente")}
                <div style={{ display: "flex", gap: "8px" }}>
                  <select value={clienteId} onChange={(e) => setClienteId(e.target.value)} style={{ flex: 1, padding: "8px 12px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "14px", boxSizing: "border-box", background: "#fff" }}>
                    <option value="">Seleccionar paciente...</option>
                    {clientes.map(c => <option key={c.id} value={c.id}>{c.apellido}, {c.nombre}</option>)}
                  </select>
                  <button type="button" onClick={() => setMostrarNuevoCliente(!mostrarNuevoCliente)} style={{ background: "#185FA5", color: "#fff", border: "none", borderRadius: "6px", padding: "8px 14px", cursor: "pointer", fontSize: "13px", whiteSpace: "nowrap" }}>
                    + Nuevo
                  </button>
                </div>
                {mostrarNuevoCliente && (
                  <div style={{ background: "#f4f7fb", border: "1px solid #dde3ec", borderRadius: "8px", padding: "14px", marginTop: "10px" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                      <input value={nuevoCliente.nombre} onChange={(e) => setNuevoCliente({ ...nuevoCliente, nombre: e.target.value })} placeholder="Nombre" style={{ padding: "8px 10px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "13px", boxSizing: "border-box" }} />
                      <input value={nuevoCliente.apellido} onChange={(e) => setNuevoCliente({ ...nuevoCliente, apellido: e.target.value })} placeholder="Apellido" style={{ padding: "8px 10px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "13px", boxSizing: "border-box" }} />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                      <input value={nuevoCliente.dni} onChange={(e) => setNuevoCliente({ ...nuevoCliente, dni: e.target.value })} placeholder="DNI" style={{ padding: "8px 10px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "13px", boxSizing: "border-box" }} />
                      <input value={nuevoCliente.telefono} onChange={(e) => setNuevoCliente({ ...nuevoCliente, telefono: e.target.value })} placeholder="Telefono" style={{ padding: "8px 10px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "13px", boxSizing: "border-box" }} />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                      <input value={nuevoCliente.email} onChange={(e) => setNuevoCliente({ ...nuevoCliente, email: e.target.value })} placeholder="Email" style={{ padding: "8px 10px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "13px", boxSizing: "border-box" }} />
                      <input type="date" value={nuevoCliente.fecha_nacimiento} onChange={(e) => setNuevoCliente({ ...nuevoCliente, fecha_nacimiento: e.target.value })} style={{ padding: "8px 10px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "13px", boxSizing: "border-box" }} />
                    </div>
                    <div style={{ marginBottom: "10px" }}>
                      <input value={nuevoCliente.obra_social} onChange={(e) => setNuevoCliente({ ...nuevoCliente, obra_social: e.target.value })} placeholder="Obra social" style={{ width: "100%", padding: "8px 10px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "13px", boxSizing: "border-box" }} />
                    </div>
                    <div style={{ marginBottom: "10px" }}>
                      <textarea value={nuevoCliente.observaciones} onChange={(e) => setNuevoCliente({ ...nuevoCliente, observaciones: e.target.value })} placeholder="Observaciones" rows={2} style={{ width: "100%", padding: "8px 10px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "13px", boxSizing: "border-box", resize: "vertical" }} />
                    </div>
                    <button type="button" onClick={crearCliente} disabled={guardandoCliente} style={{ background: "#27500A", color: "#fff", border: "none", borderRadius: "6px", padding: "8px 16px", cursor: "pointer", fontSize: "13px" }}>
