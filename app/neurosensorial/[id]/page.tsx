...
function Seccion({ titulo, abierto, onToggle, children }) {
  return (
    <div style={{ border: "1px solid #dde3ec", borderRadius: "8px", marginBottom: "10px", overflow: "hidden" }}>
      ...
      {abierto && <div style={{ padding: "20px", background: "#fff", borderTop: "1px solid #f0f2f5" }}>{children}</div>}
    </div>
  );
}
    ...
    const checkUser = async () => {
      ...
      if (ev) {
        ...
        setForm({
          ...
          sintomas_lectura: arr(ev.sintomas_lectura),
          ...
          ppc_sf: ev.ppc_sf || "",
          ppc_cf: ev.ppc_cf || "",
          seg_calidad: arr(ev.seg_calidad),
          seg_mov_cab: arr(ev.seg_mov_cab),
          seg_tiempo: arr(ev.seg_tiempo),
          seg_cognitivo: arr(ev.seg_cognitivo),
          sac_calidad: arr(ev.sac_calidad),
          sac_mov_cab: arr(ev.sac_mov_cab),
          sac_hipo_hiper: arr(ev.sac_hipo_hiper),
          sac_tiempo: arr(ev.sac_tiempo),
          campo_visual: arr(ev.campo_visual),
          equilibrio: arr(ev.equilibrio),
          linea_media: arr(ev.linea_media),
          ps: arr(ev.ps),
          fusion_ldw: arr(ev.fusion_ldw),
          ...
          diagnostico_vergencia: arr(ev.diagnostico_vergencia),
          diagnostico_acomodativo: arr(ev.diagnostico_acomodativo),
          diagnostico_oculomotor: arr(ev.diagnostico_oculomotor),
          diagnostico_estrabismo: arr(ev.diagnostico_estrabismo),
          diagnostico_ambliopia: arr(ev.diagnostico_ambliopia),
          diagnostico_perceptual: arr(ev.diagnostico_perceptual),
          diagnostico_lectura: arr(ev.diagnostico_lectura),
