...
export default function EditarVenta() {
  ...
  const handleGuardar = async () => {
    ...
    const { error } = await supabase.from("ventas").update(datos).eq("id", params.id);
    if (error) { setError("Error al guardar: " + error.message); setLoading(false); }
    else { router.push("/ventas"); }
  };

  const handleEliminar = async () => {
    if (!confirm("Estas seguro que queres eliminar esta venta?")) return;
    await supabase.from("ventas").delete().eq("id", params.id);
    router.push("/ventas");
  };
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f4f7fb" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        <div style={{ background: "#fff", borderBottom: "1px solid #dde3ec", padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: "15px", fontWeight: "600", color: "#1e3a5f" }}>Editar venta</span>
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={handleEliminar} style={{ background: "#FCEBEB", color: "#A32D2D", border: "none", borderRadius: "6px", padding: "8px 16px", cursor: "pointer", fontSize: "13px" }}>
              Eliminar
            </button>
        ...
        <div style={{ padding: "24px", maxWidth: "650px" }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #dde3ec", borderRadius: "8px", padding: "20px", display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
              ...
              <div>
                <label style={{ fontSize: "12px", color: "#6b7a8f", display: "block", marginBottom: "4px" }}>Cliente</label>
            ...
            <div>
              ...
              <input name="descripcion" value={form.descripcion} onChange={handleChange} style={{ width: "100%", padding: "8px 12px", border: "1px solid #dde3ec", borderRadius: "6px", fontSize: "14px", boxSizing: "border-box" }} />
            </div>
